...
//		EmployeePayrollService employeePayrollService;
//		employeePayrollService = new EmployeePayrollService(Arrays.asList(arrayOfEmps));
//		EmployeePayrollData[] arrPayrollData = { new EmployeePayrollData(0, "Sunder", 600000.0),
//				new EmployeePayrollData(0, "Mukesh", 1000000.0), new EmployeePayrollData(0, "Anil", 200000.0) };
...
//	}
	...
	public void givenEmployeeToDelete_WhenDeleted_ShouldMatch200ResponseAndCount() {
		...
		int statusCode = response.getStatusCode();
		Assert.assertEquals(200, statusCode);
		employeePayrollService.deleteEmployeeToPayroll("Anil", EmployeePayrollService.IOService.REST_IO);
		long entries = employeePayrollService.countEntries(EmployeePayrollService.IOService.REST_IO);
